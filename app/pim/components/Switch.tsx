...
interface CustomizedSwitchesProps {
    checked: boolean; // เพิ่ม prop checked
    onChange: (checked: boolean) => void; // callback function
  }
...
const CustomizedSwitches: React.FC<CustomizedSwitchesProps> = ({ checked, onChange }) => {
  ...
  useEffect(() => {
    setSwitchState(checked); // Update state if prop changes
  }, [checked]);
