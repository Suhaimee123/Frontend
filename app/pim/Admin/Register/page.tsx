...
import Iconrequest from "/public/request.png";
...
import RequestForm from '@/app/register/components/From/RequestForm';
...
const Register: React.FC = () => {
  ...
  const [openrequestDialog, setOpenrequestDialog] = useState(false); // For PDF view
  ...
  const initialColumns: GridColDef[] = [
    ...
    },
    {
      ...
      disableColumnMenu: true,
      renderCell: (params: { row: any }) => (
        <IconButton onClick={() => handleOpenrequest(params.row)}>
          <Image src={Iconrequest} alt="request Icon" width={24} height={24} />
        </IconButton>
      ),
    },
  ...
  const handleOpenrequest = (rowData: Student) => {
    console.log('Opening request for student:', rowData);
  ...
  const handleSearchClick = () => {
    fetchStudents(paginationModel.page, paginationModel.pageSize, studentIdSearch);
  };
  ...
  return (
    <>
      <LayoutAdmin contentTitle="สมัคร" sidebarItems={renderSidebarItems}>
        <main>
          {/* ตรงนี้ */}
          ...
          <Dialog open={openEditDialog} onClose={handleCloseEditDialog} maxWidth="lg" fullWidth>
            <DialogTitle>{selectedStudent ? 'แก้ไขข้อมูลนักศึกษา' : 'สร้างใบสมัคร'}</DialogTitle>
            <DialogContent>
          ...
          >
            <DialogContent>
              <FormProvider {...formMethods}>
                <RequestForm formMethods={formMethods} />
              </FormProvider>
            </DialogContent>
          </Dialog>
