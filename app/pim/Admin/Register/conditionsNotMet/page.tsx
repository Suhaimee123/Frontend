...
import { useStudents } from '@/hooks/Admin/useStudentsNotPassed';
...
const Register: React.FC = () => {
  const [selectedForm, setSelectedForm] = useState<FormName>('conditionsNotMet');
  ...
  const { students, loading: loadingStudents, error: studentError, totalCount, fetchStudentsNotPasseds } = useStudents(setPaginationModel);
  const formMethods = useForm<Student>();
  const { setValue } = formMethods;
  const [openEditDialog, setOpenEditDialog] = useState(false); // For edit/create student
  const [openNPdfDialog, setOpenPdfDialog] = useState(false); // For PDF view
  ...
  const initialColumns: GridColDef[] = [
    ...
    { field: 'processedBy', headerName: 'อนุมัติโดย', width: 200, headerAlign: 'center', sortable: false, disableColumnMenu: true },
  ];
  ...
  const handleDeletedIds = async (deletedIds: any[]) => {
    try {
      ...
      fetchStudentsNotPasseds(paginationModel.page, paginationModel.pageSize);
  ...
  const handleSearchClick = () => {
    fetchStudentsNotPasseds(paginationModel.page, paginationModel.pageSize, studentIdSearch);
  };
  ...
  return (
    <>
      <LayoutAdmin contentTitle="ไม่ผ่านเงือนไข" sidebarItems={renderSidebarItems}>
        <main>
          {/* ตรงนี้ */}
