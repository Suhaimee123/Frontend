...
import { useStudents } from '@/hooks/Admin/useStudentHistory';
  

const AdminTabCards = dynamic(() => import('../Register/RegisterComponents/AdminTabCards'), { ssr: false });
...
const Studenthistory: React.FC = () => {
  const [selectedForm, setSelectedForm] = useState<FormName>('Studenthistory');
  ...
  const { students, loading: loadingStudents, error: studentError, totalCount, fetchStudentHistory } = useStudents(setPaginationModel);
  ...
  const initialColumns: GridColDef[] = [
    ...
    {
      field: 'startEnd',
      headerName: 'เริ่มต้น-สิ้นสุด',
      width: 200,
      headerAlign: 'center',
      sortable: false,
      disableColumnMenu: true,
      renderCell: (params) => {
        const startMonth = params.row.startMonth;
        const endMonth = params.row.endMonth;
        return (
          <span>
            {startMonth} - {endMonth}
          </span>
    ...
    {
      field: 'period',
      headerName: 'ช่วงเวลา (เดือน)',
      width: 200,
      headerAlign: 'center',
      sortable: false,
      disableColumnMenu: true,
      renderCell: (params) => {
        const startMonth = params.row.startMonth; // เช่น '20/10/24'
        const endMonth = params.row.endMonth;    // เช่น '20/11/25'
    
        if (!startMonth || !endMonth) {
          console.error('Missing startMonth or endMonth', { startMonth, endMonth });
          return <span>ข้อมูลไม่ถูกต้อง</span>;
        }
    
        // Helper function: แปลงวันที่ในรูปแบบ 'DD/MM/YY' เป็น { year, month }
        const parseMonthYear = (dateString: string) => {
          const dateParts = dateString.split('/');
          if (dateParts.length === 3) {
            const day = parseInt(dateParts[0], 10); // วัน
            const month = parseInt(dateParts[1], 10); // เดือน
            const year = parseInt(`20${dateParts[2]}`, 10); // ปี ค.ศ.
            if (!isNaN(year) && !isNaN(month) && month >= 1 && month <= 12) {
              return { year, month, day }; // เพิ่ม day ไว้สำหรับกรณีอนาคต
            }
          }
          return null;
        };
        ...
        if (startParsed && endParsed) {
          const { year: startYear, month: startMonthNum } = startParsed;
          const { year: endYear, month: endMonthNum } = endParsed;
    
          // คำนวณผลต่างเดือน
          let diffInMonths = (endYear - startYear) * 12 + (endMonthNum - startMonthNum);
    
          // กรณีพิเศษ: หากเดือนเริ่มต้นและสิ้นสุดเท่ากัน แต่ปีต่างกัน (ถือเป็น 12 เดือน)
          if (startMonthNum === endMonthNum && startYear !== endYear) {
            diffInMonths = 12;
          }
        
          if (diffInMonths >= 0) {
            return <span>{`${diffInMonths} เดือน`}</span>;
          } else {
            return <span>ข้อมูลไม่ถูกต้อง</span>;
          }
        } else {
  ...
  const handleDeletedIds = async (deletedIds: any[]) => {
    try {
      ...
      fetchStudentHistory(paginationModel.page, paginationModel.pageSize);
  ...
  const handleSearchClick = () => {
    fetchStudentHistory(paginationModel.page, paginationModel.pageSize, studentIdSearch);
  };
  ...
  return (
    <>
      <LayoutAdmin contentTitle="ประวัติ นักศึกษา" sidebarItems={renderSidebarItems}>
        <main>
          {/* ตรงนี้ */}
