...
export const useStudents = (setPaginationModel: unknown) => {
    ...
    const fetchStudentsPasseds = async (offset: number, limit: number, studentId?: string) => {
        setLoading(true);
        setError(null);
        try {
            const response: ApiResponseS<Student[]> = await apiStudents.fetchAllStudentsPasseds(offset, limit,studentId);
            if (response.success) {
    ...
    return { students, loading, error, totalCount, fetchStudentsPasseds };
};
