...
export const useStudents = (setPaginationModel: unknown) => {
    ...
    const fetchStudents = async (offset: number, limit: number, studentId?: string) => {
        setLoading(true);
        setError(null);
        try {
            const response: ApiResponseS<Student[]> = await apiStudents.fetchAllStudents(offset, limit,studentId);
            if (response.success) {
