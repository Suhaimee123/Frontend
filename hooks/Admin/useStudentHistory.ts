...
export const useStudents = (setPaginationModel: unknown) => {
    ...
    const fetchStudentHistory = async (offset: number, limit: number, studentId?: string) => {
        setLoading(true);
        setError(null);
        try {
            const response: ApiResponseS<Student[]> = await apiStudents.fetchAllStudentHistory(offset, limit,studentId);
            if (response.success) {
    ...
    return { students, loading, error, totalCount, fetchStudentHistory };
};
